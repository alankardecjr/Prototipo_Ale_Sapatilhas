...
class JanelaCadastroVendas(tk.Toplevel):
    ...
    def _snapshot_pdv(self):
        return (
            ...
            self.cb_forma_pgto.get() if hasattr(self, "cb_forma_pgto") else "",
            self.ent_valor_pago.get() if hasattr(self, "ent_valor_pago") else "0.00",
            self.cb_parcelas.get() if hasattr(self, "cb_parcelas") else "1",
        )
    ...
    def setup_layout(self):
        ...
        for item in botoes:
            ...
            ui_utils.aplicar_hover_botao(btn, self.cor_btn_menu, paleta=ui_utils.get_paleta(), cor_ativo=self.cor_btn_menu)
            btn.pack(fill="x", pady=3, padx=5)
            if "FINALIZAR" in text_aux or "SALVAR" in text_aux:
                self.btn_finalizar = btn
                self.btn_finalizar.config(state="disabled")
                self.btn_finalizar._ativo = False
    ...
    def setup_sessao_produtos(self, parent):
        ...
        ui_utils.aplicar_estilo_foco_entry(self.ent_busca_prod, paleta=ui_utils.get_paleta())
    ...
    def setup_coluna_cliente_carrinho(self, parent):
        ...
        ui_utils.aplicar_estilo_foco_entry(self.ent_busca_cli, paleta=ui_utils.get_paleta())
        ...
        ui_utils.aplicar_estilo_foco_entry(self.ent_desconto, paleta=ui_utils.get_paleta())
        ...
        self.frame_pagamento = tk.LabelFrame(
            col_dir, text=" 💳 Pagamento / Fechamento ", bg=self.bg_card, fg=self.cor_destaque,
            font=("Segoe UI", 10, "bold"), relief="solid", borderwidth=1,
        )
        self.frame_pagamento.grid(row=3, column=0, sticky="ew", pady=4)
        self.frame_pagamento.columnconfigure(1, weight=1)
        self.frame_pagamento.columnconfigure(3, weight=1)

        tk.Label(self.frame_pagamento, text="Forma de Pagamento", font=("Segoe UI", 9, "bold"),
                 bg=self.bg_card, fg=self.cor_texto).grid(row=0, column=0, sticky="w", padx=5, pady=(8, 0))
        self.cb_forma_pgto = ttk.Combobox(
            self.frame_pagamento, values=self.FORMAS_PAGAMENTO, state="readonly",
            font=("Segoe UI", 9), width=16,
        )
        self.cb_forma_pgto.set(self.FORMAS_PAGAMENTO[0])
        self.cb_forma_pgto.grid(row=1, column=0, sticky="ew", padx=5, pady=(2, 8))
        self.cb_forma_pgto.bind("<<ComboboxSelected>>", self._on_forma_pagamento)

        tk.Label(self.frame_pagamento, text="Valor Pago (R$)", font=("Segoe UI", 9, "bold"),
                 bg=self.bg_card, fg=self.cor_texto).grid(row=0, column=1, sticky="w", padx=5, pady=(8, 0))
        self.ent_valor_pago = tk.Entry(
            self.frame_pagamento, font=("Segoe UI", 10), width=14,
            highlightthickness=1, highlightbackground=self.cor_borda,
        )
        self.ent_valor_pago.insert(0, "0.00")
        self.ent_valor_pago.grid(row=1, column=1, sticky="w", padx=5, pady=(2, 8), ipady=3)
        self.ent_valor_pago.bind("<KeyRelease>", lambda e: self.atualizar_view_carrinho())
        ui_utils.aplicar_estilo_foco_entry(self.ent_valor_pago, paleta=ui_utils.get_paleta())

        tk.Label(self.frame_pagamento, text="Parcelas", font=("Segoe UI", 9, "bold"),
                 bg=self.bg_card, fg=self.cor_texto).grid(row=0, column=2, sticky="w", padx=5, pady=(8, 0))
        self.cb_parcelas = ttk.Combobox(
            self.frame_pagamento, values=[str(i) for i in range(1, 13)], state="readonly",
            font=("Segoe UI", 9), width=8,
        )
        self.cb_parcelas.set("1")
        self.cb_parcelas.grid(row=1, column=2, sticky="w", padx=5, pady=(2, 8))
        self.cb_parcelas.bind("<<ComboboxSelected>>", lambda e: self.atualizar_view_carrinho())

        self.lbl_troco = tk.Label(
            self.frame_pagamento, text="Troco: R$ 0.00", font=("Segoe UI", 10, "bold"),
            bg=self.bg_card, fg=self.cor_texto, anchor="w",
        )
        self.lbl_troco.grid(row=2, column=0, columnspan=2, sticky="w", padx=5, pady=(0, 4))

        self.lbl_status_venda = tk.Label(
            self.frame_pagamento, text="Status: Pendente", font=("Segoe UI", 10, "bold"),
            bg=self.bg_card, fg=self.cor_destaque, anchor="w",
        )
        self.lbl_status_venda.grid(row=2, column=2, columnspan=1, sticky="w", padx=5, pady=(0, 4))

        self._on_forma_pagamento()
    ...
    def _on_busca_cliente_key(self, event=None):
        texto_atual = self.ent_busca_cli.get()
        if texto_atual == self.PLACEHOLDER_BUSCA_CLI:
            self.ent_busca_cli.delete(0, tk.END)
            self.ent_busca_cli.config(fg=self.cor_texto)
            termo = ""
        else:
            termo = texto_atual.strip()
    ...
    def _valor_pago(self):
        try:
            return max(float(self.ent_valor_pago.get().replace(",", ".")), 0)
        except (ValueError, AttributeError):
            return 0.0

    def _parcela_selecionada(self):
        try:
            return max(int(self.cb_parcelas.get()), 1)
        except (ValueError, AttributeError):
            return 1
    ...
    def _atualizar_estado_botao_finalizar(self):
        ...
        if hasattr(self, "btn_finalizar"):
            ...
            self.btn_finalizar._ativo = bool(ok)
            self.btn_finalizar.config(bg=self.cor_destaque if ok else self.cor_btn_menu)

    def _on_forma_pagamento(self, event=None):
        forma = self.cb_forma_pgto.get()
        if forma in ("Cartão de Crédito", "Crediário"):
            self.cb_parcelas.config(state="readonly")
            if int(self.cb_parcelas.get() or "1") < 1:
                self.cb_parcelas.set("1")
        else:
            self.cb_parcelas.set("1")
            self.cb_parcelas.config(state="disabled")

        if forma == config.FORMA_PAGAMENTO_PDV_PENDENTE:
            self.ent_valor_pago.delete(0, tk.END)
            self.ent_valor_pago.insert(0, "0.00")
            self.ent_valor_pago.config(state="disabled")
        else:
            self.ent_valor_pago.config(state="normal")
        self.atualizar_view_carrinho()
    ...
    def filtrar_produtos(self):
        termo = self.ent_busca_prod.get()
        if termo == self._placeholder_prod:
            termo = ""
        termo = termo.lower()
        self.tree_estoque.delete(*self.tree_estoque.get_children())
        for p in self._estoque_cache:
    ...
    def atualizar_view_carrinho(self):
        ...
        valor_pago = self._valor_pago()
        troco = max(valor_pago - total, 0)
        status = "Pago" if total > 0 and valor_pago >= total - 0.01 else "Pendente"
        ...
        self.lbl_troco.config(text=f"Troco: R$ {troco:.2f}")
        self.lbl_status_venda.config(text=f"Status: {status}")
        self._atualizar_estado_botao_finalizar()
    ...
    def finalizar_venda(self):
        """Registra a venda no PDV e trata o pagamento direto quando informado."""
        ...
        total = self._total_liquido()
        forma_pgto = self.cb_forma_pgto.get() if hasattr(self, "cb_forma_pgto") else config.FORMA_PAGAMENTO_PDV_PENDENTE
        valor_pago = self._valor_pago() if hasattr(self, "ent_valor_pago") else 0.0
        parcelas = self._parcela_selecionada() if hasattr(self, "cb_parcelas") else 1
        ...
        if self.venda_id:
            ...
            if not res:
                ...
                return
            vid = self.venda_id
            messagebox.showinfo("Sucesso", "Venda atualizada.", parent=self)
            if hasattr(self.master, "atualizar_lista"):
                self.master.atualizar_lista()
        ...
        if forma_pgto == config.FORMA_PAGAMENTO_PDV_PENDENTE and valor_pago > 0:
            messagebox.showwarning(
                "Aviso",
                "Forma de pagamento pendente não aceita valor pago. Informe a forma correta ou deixe o valor em 0.",
                parent=self,
            )
            return

        if valor_pago >= total - 0.01 and forma_pgto != config.FORMA_PAGAMENTO_PDV_PENDENTE:
            res, msg, vid = database.realizar_venda_pdv(
                self.cliente_atual[0], lista_final, desconto,
                forma_pagamento=forma_pgto, valor_pago=valor_pago, parcelas=parcelas,
            )
            if not res:
                messagebox.showerror("Erro", msg, parent=self)
                return
            ...
            if hasattr(self.master, "atualizar_lista"):
                self.master.atualizar_lista()
            message = f"Venda finalizada e paga. Total: R$ {total:.2f}."
            if troco > 0:
                message += f" Troco: R$ {troco:.2f}."
            messagebox.showinfo("PDV", message, parent=self)
        else:
            res, msg, vid = database.realizar_venda_pdv(
                self.cliente_atual[0], lista_final, desconto,
                forma_pagamento=forma_pgto, parcelas=parcelas,
            )
            if not res:
                messagebox.showerror("Erro", msg, parent=self)
                return
    ...
    def _limpar_exceto_produtos(self):
        ...
        if hasattr(self, "ent_desconto"):
            self.ent_desconto.delete(0, tk.END)
            self.ent_desconto.insert(0, "0.00")
        if hasattr(self, "cb_forma_pgto"):
            self.cb_forma_pgto.set(self.FORMAS_PAGAMENTO[0])
        if hasattr(self, "cb_parcelas"):
            self.cb_parcelas.set("1")
            self.cb_parcelas.config(state="disabled")
        if hasattr(self, "ent_valor_pago"):
            self.ent_valor_pago.delete(0, tk.END)
            self.ent_valor_pago.insert(0, "0.00")
        if hasattr(self, "lbl_troco"):
            self.lbl_troco.config(text="Troco: R$ 0.00")
        if hasattr(self, "lbl_status_venda"):
            self.lbl_status_venda.config(text="Status: Pendente")
        self.txt_cli_detalhes.config(state="normal")
        self.txt_cli_detalhes.delete("1.0", tk.END)
